...
trait PayrollTrait
{
    ...
    public function start_payroll_approval(Request $request)
    {
        ...
        if ($pp->uses_approval == 1 & $PR->workflow_id > 0) {
            ...
            if ($stage->type == 1) {
                $PR->payroll_approvals()->create(['stage_id' => $stage->id, 'comments' => '', 'status' => 0, 'approver_id' => $stage->user_id
                ]);
                if ($stage->user) {
...
//             return view('compensation.partials.navpayroll_int',compact('sections','payroll','chart_of_accounts','payroll_details','date','specific_salary_component_types', 'allusers','pp','lsas'));
    ...
    public function payrollList(Request $request)
    {
        ...
        $section_name=\App\UserSection::find($request->section)->name;

        $payroll = Payroll::where(['month' => $pmonth, 'year' => $pyear, 'company_id' => $company_id,'section_id'=>$request->section])->first();
        ...
        if ($payroll) {
            ...
            return view('compensation.payroll', compact('payroll', 'allowances', 'deductions', 'income_tax', 'salary', 'date', 'has_been_run','sections','section','section_name'));
        } else {
            $has_been_run = 0;
            ...
            return view('compensation.payroll', compact('date', 'employees', 'has_been_run','sections','section','section_name'));
    ...
    public function runPayroll(Request $request)
    {
        ...
        \Artisan::queue('payroll:run', ['month' => $request->month, 'company_id' => $company_id, 'creator_id' => $creator_id, 'thread_id' => $thread_id,'section'=>$request->section]);
    ...
    public function commandrunPayroll($request)
    {
        ...
        $users = User::has('promotionHistories.grade')->where('company_id', $company_id)->where('payroll_type', 'office')->where('section_id',$section->id)->where('status', '!=', 2)->orWhereHas('separations', function ($query) use ($date) {
            $query->where('separations.date_of_separation', '>=', $date)
                ->where('users.payroll_type', 'office');

        })->with('separations')->get();
       
        $tmsa_users = User::where('company_id', $company_id)->where('payroll_type', 'tmsa')->where('status', '!=', 2)->where('section_id',$section->id)->orWhereHas('separations', function ($query) use ($date) {
            $query->where('separations.date_of_separation', '>=', $date)
                ->where('users.payroll_type', 'tmsa');

        })->with('separations')->get();
        $project_users = User::where('company_id', $company_id)->where('payroll_type', 'project')->where('status', '!=', 2)->where('section_id',$section->id)->orWhereHas('separations', function ($query) use ($date) {
            $query->where('separations.date_of_separation', '>=', $date)
        ...
        $payroll = Payroll::where(['month' => $pmonth, 'year' => $pyear, 'company_id' => $company_id,'section_id'=>$section->id])->first();
        $pp = PayrollPolicy::where('company_id', $company_id)->first();
        ...
        if ($payroll) {
            ...
            if ($pp->use_office == 1 || $pp->use_tmsa == 1 || $pp->use_project == 1) {
                ...
                $PR = Payroll::create(['month' => $pmonth, 'year' => $pyear, 'company_id' => $company_id, 'workflow_id' => $pp->workflow->id, 'for' => $date, 'user_id' => $creator_id,'section_id'=>$section->id]);
...
//        	$payroll['paye']=$payroll['annual_paye']/12;
    ...
    public function projectGrosspay($payroll)
    {
        ...
        foreach ($components as $component) {
            ...
            if ($component->status == 1 && $component->exemptions->contains('id', $user_id) == false && $component->type == 1 && (($component->uses_anniversary == 1 && $payroll['is_anniversary'] == 1) || ($component->uses_anniversary == 0))) {
    ...
    public function projectAllowancesAndDeductions(&$payroll)
    {
        ...
        if ($salary_category->uses_daily_net == 1) {
            ...
            foreach ($components as $component) {
                ...
                if ($component->status == 1 && $component->exemptions->contains('id', $user_id) == false && $component->constant != 'daily_net' && (($component->uses_anniversary == 1 && $payroll['is_anniversary'] == 1) || ($component->uses_anniversary == 0))) {
            ...
            foreach ($components as $component) {
                ...
                if ($component->status == 1 && $component->exemptions->contains('id', $user_id) == false && (($component->uses_anniversary == 1 && $payroll['is_anniversary'] == 1) || ($component->uses_anniversary == 0))) {
    ...
    public function calculateProjectPAYE(&$payroll)
    {
        ...
        if ($user->project_salary_category) {
            ...
            $payroll['taxable_income'] = $payroll['tax_gross_pay'] - $payroll['consolidated_allowance'] - ($user->project_salary_category->relief * 12) - ($sum_of_rebate * 12) - ($payroll['not_taxable'] * 12);// + ($payroll['ssc_total_allowances']*12);
            if ($user->project_salary_category->uses_tax == 1 & $user->project_salary_category->uses_direct_tax==0) {
                $payroll['tax_gross_pay_without_annual_allowances'] = ($payroll['gross_pay']) + ($payroll['ssc_monthly_taxable_all'] * 12);
                $payroll['consolidated_allowance_without_annual_allowances'] = $this->consolidated_allowance($payroll['tax_gross_pay_without_annual_allowances']);

                $payroll['taxable_income_without_annual_allowances'] = $payroll['tax_gross_pay_without_annual_allowances'] - $payroll['consolidated_allowance_without_annual_allowances'] - (($payroll['not_taxable']) * 12) - ($sum_of_rebate * 12) - ($user->project_salary_category->relief * 12);
                $this->calculate_tax_without_annual_allowances($payroll);
                $this->calculate_tax($payroll);
